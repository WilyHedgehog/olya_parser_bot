...
class NatsKeyBuilder(KeyBuilder):
    def __init__(self, prefix: str = "fsm") -> None:
        self.prefix = prefix

    def build(self, key: StorageKey) -> str:
        # StorageKey содержит bot_id, chat_id, user_id
        parts = [str(key.bot_id), str(key.chat_id), str(key.user_id)]
        # Склеиваем через "_", чтобы ключ был валидный для JetStream
