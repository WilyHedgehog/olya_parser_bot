...
async def admin_mailing(
    ...
    users = await get_all_users_in_segment(
        scheduled.segment
    )  # список chat_id пользователей в сегменте
...
async def set_admin_mailing(
    mailing_datetime, message, file_id, keyboard, segment, task_name
):
